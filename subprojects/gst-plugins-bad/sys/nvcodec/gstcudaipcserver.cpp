...
GST_DEBUG_CATEGORY (gst_cuda_ipc_server_debug);
#define GST_CAT_DEFAULT gst_cuda_ipc_server_debug
...
G_DEFINE_ABSTRACT_TYPE_WITH_CODE (GstCudaIpcServer,
    ...
    GST_DEBUG_CATEGORY_INIT (gst_cuda_ipc_server_debug, "cudaipcserver",
        0, "cudaipcserver"));
    ...
    const std::vector < std::string > &custom_metas)
{
  ...
  data->handle = handle;
  data->pts = pts;
  data->custom_metas = custom_metas;
