...
G_DEFINE_ABSTRACT_TYPE_WITH_CODE (GstCudaIpcClient, gst_cuda_ipc_client,
    GST_TYPE_OBJECT,
    GST_DEBUG_CATEGORY_INIT (gst_cuda_ipc_client_debug, "cudaipcclient",
        0, "cudaipcclient"));
...
gst_cuda_ipc_client_have_data (GstCudaIpcClient * self)
{
  ...
  if (!gst_cuda_ipc_pkt_parse_have_data (conn->server_msg,
          pts, layout, handle, &caps, custom_metas)) {
  ...
  GST_LOG_OBJECT (self, "have %" G_GSIZE_FORMAT " custom meta",
      custom_metas.size ());

  /* *INDENT-OFF* */
  for (auto it : custom_metas)
    gst_buffer_add_meta_from_string (buffer, it.c_str ());
  /* *INDENT-ON* */
