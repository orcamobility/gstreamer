...
GST_DEBUG_CATEGORY_STATIC (gst_cuda_ipc_sink_debug);
#define GST_CAT_DEFAULT gst_cuda_ipc_sink_debug
...
struct GstCudaIpcSinkPrivate
{
  ...
  std::vector<std::string> custom_metas;
...
gst_cuda_ipc_sink_class_init (GstCudaIpcSinkClass * klass)
{
...
#else
          "Path to unix domain socket used to communicate with clients",
#endif
  ...
  GST_DEBUG_CATEGORY_INIT (gst_cuda_ipc_sink_debug, "cudaipcsink",
      0, "cudaipcsink");
}
    ...
    GstCudaIpcSink * self)
{
  ...
  priv->custom_metas.emplace_back (str);
  g_free (str);
...
gst_cuda_ipc_sink_prepare (GstBaseSink * sink, GstBuffer * buf)
{
  ...
  GST_TRACE_OBJECT (self, "Exported handle value for %" G_GUINTPTR_FORMAT " %s",
      ptr, handle_dump.c_str ());

  priv->prepared_sample = gst_sample_new (cuda_buf,
      priv->caps, nullptr, nullptr);

  priv->custom_metas.clear ();
...
gst_cuda_ipc_sink_render (GstBaseSink * sink, GstBuffer * buf)
{
  ...
  pts = now_system = gst_cuda_ipc_clock_now ();
  ...
  ret = gst_cuda_ipc_server_send_data (priv->server, priv->prepared_sample,
      priv->mem_info, priv->prepared_handle, pts, priv->custom_metas);
  gst_clear_sample (&priv->prepared_sample);
  priv->custom_metas.clear ();
